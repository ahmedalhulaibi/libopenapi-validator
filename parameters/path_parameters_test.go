...
func TestValidatePathParams_FragmentPath_NoPanic(t *testing.T) {
	// Regression: specs with fragment-annotated paths (e.g., AWS APIs)
	// caused a panic in ValidatePathParamsWithPathItem because
	// StripRequestPath appends the URL fragment to the path, creating a
	// segment mismatch with the spec path segments.
	spec := `openapi: 3.1.0
info:
  title: test
  version: "1.0"
paths:
  /devices/{deviceId}/events#fromTimeStamp&toTimeStamp:
	...
	// Simulate a request with a URL fragment (as generated by rapidoas).
	request, _ := http.NewRequest(http.MethodGet,
		"https://example.com/devices/abc123/events?fromTimeStamp=2020-01-01&toTimeStamp=2020-12-31#fromTimeStamp&toTimeStamp",
		nil,
	)

	// Must not panic.
	valid, validationErrors := v.ValidatePathParams(request)
	assert.True(t, valid, "path params should validate: %v", validationErrors)
